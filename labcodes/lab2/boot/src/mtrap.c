...
__attribute__((section("htif")));
volatile uint64_t fromhost __attribute__((aligned(64)))
__attribute__((section("htif")));
static spinlock_t htif_lock = SPINLOCK_INIT;

void __attribute__((noreturn)) bad_trap() {
    die("machine mode: unhandlable trap %d @ %p", read_csr(mcause),
        read_csr(mepc));
}

static uintptr_t mcall_hart_id() { return read_const_csr(mhartid); }
...
static uintptr_t mcall_send_ipi(uintptr_t recipient) {
    // if (recipient >= num_harts) return -1;

    // send_ipi(recipient, IPI_SOFT);
    // This function is used as a interface to enable paging
    putstring("Alive on entering mcall_send_ipi\n");
    if (recipient != 0) {
        uintptr_t ms = read_csr(mstatus);
        ms = INSERT_FIELD(ms, MSTATUS_VM, VM_SV32);
        write_csr(mstatus, ms);
    }
    putstring("Alive on exiting mcall_send_ipi\n");
    return 0;
}
...
static void reset_ssip() {
    ...
    if (HLS()->sipi_pending || HLS()->console_ibuf > 0) set_csr(mip, MIP_SSIP);
}

static uintptr_t mcall_console_getchar() {
    int ch = atomic_swap(&HLS()->console_ibuf, -1);
    if (ch >= 0) request_htif_keyboard_interrupt();
    reset_ssip();
    return ch - 1;
}
...
static uintptr_t mcall_shutdown() { poweroff(); }
...
void software_interrupt() {
    *HLS()->ipi = 0;
    ...
    if (ipi_pending & IPI_FENCE_I) asm volatile("fence.i");

    if (ipi_pending & IPI_SFENCE_VM) asm volatile("sfence.vm");
}
...
static void send_ipi_many(uintptr_t* pmask, int event) {
    ...
    for (size_t i = num_harts - 1; i >= 0; i--)
        if ((mask >> i) & 1) send_ipi(i, event);

    // wait until all events have been handled.
    // prevent deadlock while spinning by handling any IPIs from other harts.
    for (size_t i = num_harts - 1; i >= 0; i--)
        if ((mask >> i) & 1)
            while (OTHER_HLS(i)->mipi_pending & event) software_interrupt();
}
...
void redirect_trap(uintptr_t epc, uintptr_t mstatus) {
    ...
    uintptr_t new_mstatus =
        mstatus & ~(MSTATUS_SPP | MSTATUS_SPIE | MSTATUS_MPIE);
