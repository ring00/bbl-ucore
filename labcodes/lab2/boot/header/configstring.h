    ...
    while (1) {
        ...
        while (*str != ';') {
            if (!*str) {
                ...
                if (inner_res.start) return inner_res;
                str = inner_res.end + 1;
            } else {
                str = skip_key(str);
            }
...
static void parse_string(query_result r, char* buf) {
    if (r.start < r.end) {
        if (*r.start == '"') {
            ...
            for (const char* p = r.start; p<r.end&& * p> ' '; p++) *buf++ = *p;
        }
    }
    *buf = 0;
}
...
static uint64_t __get_uint(const char* s) {
    if (s[0] == '0' && s[1] == 'x') return __get_uint_hex(s + 2);
