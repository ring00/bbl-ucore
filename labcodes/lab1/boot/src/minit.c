...
static void mstatus_init() {
    ...
    // Enable FPU and set VM mode to mbare
    uintptr_t ms = 0;
    ...
    ms = read_csr(mstatus);
    assert(EXTRACT_FIELD(ms, MSTATUS_VM) == VM_MBARE);
...
static void delegate_traps() {
    ...
    uintptr_t exceptions = (1U << CAUSE_MISALIGNED_FETCH) |
                           ...
                           (1U << CAUSE_BREAKPOINT) | (1U << CAUSE_FAULT_LOAD) |
                           (1U << CAUSE_FAULT_STORE) |
                           (1U << CAUSE_BREAKPOINT) | (1U << CAUSE_USER_ECALL);
...
#ifdef __riscv_flen
    for (int i = 0; i < 32; i++) init_fp_reg(i);
    write_csr(fcsr, 0);
#else
...
static void plic_init() {
    for (size_t i = 1; i <= plic_ndevs; i++) plic_priorities[i] = 1;
}
...
static void hart_plic_init() {
    ...
    *HLS()->timecmp = -1ULL;
    ...
    if (!plic_ndevs) return;

    size_t ie_words = plic_ndevs / sizeof(uintptr_t) + 1;
    for (size_t i = 0; i < ie_words; i++)
        HLS()->plic_s_ie[i] = (unsigned long)-1;
    *HLS()->plic_m_thresh = 1;
    *HLS()->plic_s_thresh = 0;
}
