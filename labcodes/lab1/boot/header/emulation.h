...
typedef void (*emulation_func)(uintptr_t*, uintptr_t, uintptr_t, uintptr_t,
                               insn_t);
#define DECLARE_EMULATION_FUNC(name)                             \
    void name(uintptr_t* regs, uintptr_t mcause, uintptr_t mepc, \
              uintptr_t mstatus, insn_t insn)
...
#define IMM_S(insn) \
    (((int32_t)(insn) >> 25 << 5) | (int32_t)(((insn) >> 7) & 0x1f))
...
#define SET_F32_REG(insn, pos, regs, val)                                     \
    ({                                                                        \
        ...
        uintptr_t tmp;                                                        \
        asm volatile(                                                         \
            "1: auipc %0, %%pcrel_hi(put_f32_reg); add %0, %0, %2; jalr t0, " \
            "%0, %%pcrel_lo(1b)"                                              \
            : "=&r"(tmp)                                                      \
            : "r"(value), "r"(offset)                                         \
            : "t0");                                                          \
...
#define SET_F64_REG(insn, pos, regs, val)                                     \
    ({                                                                        \
        uint64_t __val = (val);                                               \
        register uintptr_t value asm("a0") =                                  \
            sizeof(uintptr_t) == 4 ? (uintptr_t)&__val : (uintptr_t)__val;    \
        uintptr_t offset = ((insn) >> ((pos)-3)) & 0xf8;                      \
        uintptr_t tmp;                                                        \
        asm volatile(                                                         \
            "1: auipc %0, %%pcrel_hi(put_f64_reg); add %0, %0, %2; jalr t0, " \
            "%0, %%pcrel_lo(1b)"                                              \
            : "=&r"(tmp)                                                      \
            : "r"(value), "r"(offset)                                         \
            : "t0");                                                          \
...
#else
#define GET_F64_REG(insn, pos, regs) \
    (*(int64_t*)((void*)((regs) + 32) + (((insn) >> ((pos)-3)) & 0xf8)))
...
#define SET_FCSR(value) \
    ({ asm volatile("add tp, x0, %0" ::"rI"((value)&0xFF)); })
...
#define softfloat_raiseFlags(which) \
    ({ asm volatile("or tp, tp, %0" ::"rI"(which)); })
#define softfloat_roundingMode     \
    ({                             \
        register int tp asm("tp"); \
        tp >> 13;                  \
